...
class CompileVendorLib extends Feng
{
    ...
    private function parseContent($content)
    {
        ...
        $content = preg_replace_callback($patternInclude, array(__CLASS__, 'replaceInclude'), $content);
        ...
        $content = preg_replace_callback($patternTrim, array(__CLASS__, 'trimContent'), $content);
        $content = preg_replace_callback($pattern, array(__CLASS__, 'preReplace'), $content);
        $content = preg_replace_callback($patternCache, array(__CLASS__, 'cacheReplace'), $content);
        return $content;
    }
        ...
        if (strpos($content, '|'))
        {
            ...
            if ($tempArr)
            {
                ...
                if ($arr && is_array($arr))
                {
                    ...
                    {
                        $tt[$key . 'E!!@@##$$E'] = $val;
                        $other = str_replace($val, $key . 'E!!@@##$$E', $other);
                    }
                }
            }
            $arr = explode(':', $other);
    ...
    private function toTime($timeStr)
    {
