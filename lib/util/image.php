...
class ImageUtilLib extends Feng
{
    ...
    function waterMark($groundName, $waterName, $waterPos = 0, $qz = "wa_")
    {
        if (file_exists($groundName) && file_exists($waterName))
        {
            ...
            $groundImg = $this->copyImage($groundImg, $waterImg, $pos, 
                    $waterInfo); // 拷贝图像
    ...
    {
        imagecopy($groundImg, $waterImg, $pos["posX"], $pos["posY"], 0, 0, 
    ...
    private function kidOfImage($srcImg, $size, $imgInfo)
    {
        ...
        { // 取得一幅图像的调色板中颜色的数目
            ...
            $newtransparentcolor = imagecolorallocate($newImg, 
                    $transparentcolor['red'], $transparentcolor['green'], 
                    $transparentcolor['blue']);
            
            imagefill($newImg, 0, 0, $newtransparentcolor);
            imagecolortransparent($newImg, $newtransparentcolor);
        }
        imagecopyresized($newImg, $srcImg, 0, 0, 0, 0, $size["width"], 
                $size["height"], $imgInfo["width"], $imgInfo["height"]);
        imagedestroy($srcImg);
        return $newImg;
    }
}