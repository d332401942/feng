...
class ConnectMysqlDbLib extends Feng
{
    ...
    public static function connectMysql($host, $userName, $passWord, $dbName, $port = 3306, 
            $options = null)
    {
        if (empty(self::$pdo[$host][$dbName]))
        {
            ...
            self::$pdo[$host][$dbName] = new PDO($dsn, $userName, $passWord, 
                    $options);
            self::$pdo[$host][$dbName]->setAttribute(
                    PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_CLASS);
            self::$pdo[$host][$dbName]->setAttribute(
                    PDO::ATTR_STRINGIFY_FETCHES, FALSE);
            self::$pdo[$host][$dbName]->setAttribute(PDO::ATTR_ERRMODE, 
